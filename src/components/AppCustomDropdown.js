...
export default function AppCustomDropdown(props) {
  ...
  return (
    <View>
      ...
      <Pressable
        ...
        onPress={() => props.handleOpenSingleDropdown(props.name)}
        style={[
          styles.select,
          {
        ...
        <AppText
          style={[
            styles.label,
            {
              color: getValue(props, `request.${props.name}.id`, '')
                ? isThemeDark
          ...
          {getValue(props, `request.${props.name}.id`, '')
            ? getValue(props, `request.${props.name}.name`, '')
